...
int main ()
{
	...
	if (pid_1 < 0)  goto X;
        ...
        //The parent process creates the second child process
        if(pid_1 != 0) pid_2 = fork();
        //The parent process tests whether fork() failed or not, if it did, the process exits.
        if ((pid_1 != 0) && (pid_2 < 0))  goto X;

	//The parent process creates the third child process
	...
	if(pid_1==0 || pid_2==0 || pid_3==0) 		//Child processes execute this segment of code
	{
		...
		//E.g., in my case it is: execl("/home/jeffery/Desktop/Labs/L1/count.out","./count.out", NULL);
		execl("X","Y", NULL);
